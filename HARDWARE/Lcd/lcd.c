...
{
    //regval =regval  使用-o2优化时, f4系列不加可能白屏, 它的fsmc速率较快
    LCD->LCD_REG = regval; // 写入要写的寄存器序号  RS信号决定传送的是数据还是命令  RS=0 命令
    // FSMC硬件自动控制时序
}
...
{
    LCD->LCD_RAM = data; // RS信号决定传送的是数据还是命令  RS=1  数据
}
...
void LCD_Init(void)
{
    ...
    ReadTiming.FSMC_AddressSetupTime = 0x01;   //地址建立时间（ADDSET）为2个HCLK 1/36M=27ns
    ReadTiming.FSMC_AddressHoldTime = 0x00;    //地址保持时间（ADDHLD）模式A未用到
    ReadTiming.FSMC_DataSetupTime = 0x0f;      //数据保存时间为16个HCLK,因为液晶驱动IC的读数据的时候，速度不能太快，尤其对1289这个IC。
    ...
    FSMC_NORSRAMInitStructure.FSMC_ReadTimingStruct = &ReadTiming;        //读写时序
    ...
    lcddev.id = LCD_RD_DATA();          // 读取0X93 ---> 1001 0011
    lcddev.id <<= 8;                    // 1001 0011 xxxx   0100 0001
